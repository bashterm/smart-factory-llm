...
class BuildTrafficPlan():
  ...
  def __init__(self, procedure, machines, m, plan, num_epochs, timesteps_per_epoch):
    ...
    self.at  = defaultdict(dict)
    self.has = defaultdict(dict)
    ...
    for road in m.roads:
      ...
      for zj in procedure.tokens:
        ...
        while to_add > 0:
          ...
          self.at[self.t][agent_cell] = agent_id
          self.has[self.t][agent_id]  = zj

          # print(f"agent_id:{agent_id}, at:{agent_cell}, has:{self.has[self.t][agent_id]}")  
    ...
    # input(f"self.total_agents:{self.total_agents}")
  ...
  def timestep(self, procedure, machines, m, plan, num_epochs, timesteps_per_epoch):
    ...
    # Plot status at start of epoch
    self.visualizer.generate_state(self.at[self.t], self.has[self.t])
    self.print_status_at_start_of_timestep(self.t, self.T, procedure, machines, m, plan, num_epochs)

    # if self.t > num_epochs*timesteps_per_epoch:

    #   self.print_status_at_start_of_timestep(self.t - num_epochs*timesteps_per_epoch, 
    #                                          self.T - num_epochs, procedure, machines, m, plan, num_epochs)
    #   input("Mirrored?")

    # Move agents on junctions
    for junction in m.junctions:
      self.move_agents_on_junction(junction, procedure, machines, m, plan)
  ...
  def initialize_epoch_plan(self, procedure, machines, m, plan, num_epochs):
    ...
    for road, zj in product(m.roads, procedure.tokens):
      self.inbound[(road.id, self.T, zj)]   = int(plan.var[ilp.inbound_var(road.id, target_T, zj)].x)
    ...
    for mid in machines.supported_processes.keys():
      ...
      for zj in machines.possibly_consumed_tokens[mid]:
        self.deposited[(mid, self.T, zj)] = int(plan.var[ilp.deposit_var(mid, target_T, zj)].x)
      ...
      for zj in machines.possibly_produced_tokens[mid]:
        self.pickedup[(mid, self.T, zj)] = int(plan.var[ilp.pickup_var(mid, target_T, zj)].x)
    

  # Moves the agent on a junction
  def move_agents_on_junction(self, junction, procedure, machines, m, plan):

    if not junction.pt in self.at[self.t]:
      return

    agent_id = self.at[self.t][junction.pt]

    for road in junction.rOut:
      if self.inbound[(road.id, self.T, self.has[self.t][agent_id])] > 0:

        # Update agent position
        self.at[self.t+1][road.path[0]] = agent_id

        # Reduce the number of agents that need to enter the road this epoch
        self.inbound[(road.id, self.T, self.has[self.t][agent_id])] -= 1

        # Note that the agent transitioned on this epoch
        self.last_transition[agent_id] = self.T
        return

    input(f"Agent {agent_id} on junction {junction.id} not needed for any next road")
  ...
  def move_agents_on_road(self, road):
    ...
    for cell in reversed(road.path):
      ...
      if cell in self.at[self.t]:
        agent_id = self.at[self.t][cell]
        ...
        if self.last_transition[agent_id] == self.T and next_cell == road.J_en.pt:
          self.at[self.t+1][cell]      = agent_id

        # Agent waits on its current cell if its next cell is occupied
        elif next_cell in self.at[self.t+1]:
          self.at[self.t+1][cell]      = agent_id

        # Otherwise, agent moves to its next cell
        else:
          self.at[self.t+1][next_cell] = agent_id
  ...
  def deposit_tokens(self, machines, m):
    ...
    for road in m.roads:
      for cell, mid in road.input_cell_to_mid.items():
        ...
        if cell not in self.at[self.t+1]:
          continue

        # Agent on cell
        agent_id = self.at[self.t+1][cell]  

        # If agent has already exchanged token this timestep, don't exchange again


        # If the agent is not carrying a token that mid needs, continue
        if self.has[self.t][agent_id] not in machines.possibly_consumed_tokens[mid]:
          continue
        ...
        if self.deposited[(mid, self.last_transition[agent_id], self.has[self.t][agent_id])] > 0:
          self.deposited[(mid, self.last_transition[agent_id], self.has[self.t][agent_id])] -= 1
          self.has[self.t+1][agent_id] = self.null_token

          print((f"Update: Agent {agent_id} token {self.has[self.t][agent_id]} "
                 f"dep({mid}, {self.last_transition[agent_id]}) cell {cell}"))
  ...
  def pickup_tokens(self, machines, m):
    ...
    for road in m.roads:
      for cell, mid in road.output_cell_to_mid.items():
        ...
        if self.has[self.t][agent_id] != self.null_token:
          continue
        ...
        for zj in machines.possibly_produced_tokens[mid]:
          if self.pickedup[(mid, self.last_transition[agent_id], zj)] > 0:
            self.has[self.t+1][agent_id] = zj
            self.pickedup[(mid, self.last_transition[agent_id], zj)] -= 1

            print((f"Update: Agent {agent_id} token {self.has[self.t+1][agent_id]} "
                   f"pck({mid}, {self.last_transition[agent_id]}) cell {cell}"))
            break
  ...
  def keep_tokens(self):
    for agent_id in range(self.total_agents):
      if agent_id not in self.has[self.t+1]:
        self.has[self.t+1][agent_id] = self.has[self.t][agent_id]


  # Make sure that no agent has vanished
  def check_for_vanishing_agents(self):
    assert len(self.at[self.t+1])  == self.total_agents
    assert len(self.has[self.t+1]) == self.total_agents


  # Print starting timestep details
  def print_status_at_start_of_timestep(self, t, T, procedure, machines, m, plan, num_epochs):
    print(f"At start of t={t}, T={T}")
    print(f"{self.at[t]}")

    for cell, agent_id in sorted(self.at[t].items(), key=lambda item: item[1]):
      print(f"Agent {agent_id} on cell {cell} with token {self.has[t][agent_id].name}")


    target_t = T % num_epochs

    print(f"---- t={t}, T={T} ({target_t}) inbound ----")

    for road in m.roads:
      r_status_str  = f"Road {road.id} needs "

      for zj in procedure.tokens:
        tokens_needed       = abs(self.inbound[(road.id, T, zj)])
    ...
    print(f"---- t={t}, T={T} ({target_t}) tokens ----")
    ...
    for road in m.roads:
      ...
      for cell, mid in road.input_cell_to_mid.items(): 
        for thisT in [T, T-1 % num_epochs]:
          for zj in machines.possibly_consumed_tokens[mid]:

            if thisT == -1:
              break

            to_dep       = abs(self.deposited[(mid, thisT, zj)])
            total_to_dep = abs(plan.var[ilp.deposit_var(mid, target_t, zj)].x)

            if total_to_dep > 0:
              r_token_str += f"{zj.name:} {to_dep:.0f}/{total_to_dep:.0f} dep({mid},{thisT})  " 
      ...
      for cell, mid in road.output_cell_to_mid.items(): 
        for thisT in [T, T-1 % num_epochs]:
          for zj in machines.possibly_produced_tokens[mid]:
            ...
            to_pck       = abs(self.pickedup[(mid, thisT, zj)])
            total_to_pck = abs(plan.var[ilp.pickup_var(mid, target_t, zj)].x)

            if total_to_pck > 0:
              r_token_str += f"{zj.name:} {to_pck:.0f}/{total_to_pck:.0f} pck({mid},{thisT})   " 
